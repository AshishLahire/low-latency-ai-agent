...
def _build_system_prompt(customer_info: str, rag_context: str) -> str:
    ...
    if customer_info:
        base += (
            f"CUSTOMER ON THE LINE (use this data to answer order/tracking questions):\n"
            f"{customer_info}\n\n"
            "You have the customer's full order history above. "
            "When they ask about an order, status, or tracking — refer directly to that data. "
            "Do NOT say you cannot look up orders — you already have them listed above.\n\n"
            )
    else:
        base += (
            "CUSTOMER NOT IDENTIFIED YET.\n"
            "If the user asks about their order or account, politely ask for "
            "their registered email address. Once they provide it, their data "
            "will be available.\n\n"
        )
    ...
    base += (
        ...
        "- Use ONLY the order data provided above — do not guess\n"
        "- If a specific order ID is not in the list, say it was not found\n"
